...
1. **Telescope 深度性能优化**
   - 修复了排除规则失效的问题：将 `file_ignore_patterns` 迁移至 ripgrep 的 `--glob` 原生参数。
   - 搜索范围从 **24万个文件** 缩减至有效源码范围，响应极速。
   - 细化搜索策略：默认使用极速模式 (`<Leader>ff`)，支持隐藏文件模式 (`<Leader>fh`)。

2. **Which-Key v3 架构迁移**
   - 将配置从废弃的 v2 API 完全迁移到 v3 API (`wk.add`)。
   - 应用 `modern` 现代化 UI 预设。

3. **Legendary 兼容性修复**
   - 移除了会导致运行时错误的过时 `extensions.which_key` 配置。
